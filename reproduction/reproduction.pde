...
void drawStuff() {
  for(int i = 0; i < amountOfSets; i++) {
    ...
    for(int j = 0; j < amountOfLinesInSet; j++) {
      ...
      // Versatz zum Startpunkt addieren
      xPositionLineStart = int(xPositionSet + backfillX);
      yPositionLineStart = int(yPositionSet + backfillY);

      // Linie zeichnen
      line(xPositionLineStart, yPositionLineStart, xPositionLineStart + xValLine, yPositionLineStart + yValLine);
